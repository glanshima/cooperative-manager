...
### C3. Framework preset
- Vercel auto-detects FastAPI apps at supported entrypoints — `backend/app/main.py`
  matches the pattern (`main.py` inside an `app/` directory), so **no
  `runtime` field is needed** in `vercel.json`. The scaffold's
  `backend/vercel.json` only sets `maxDuration`:
  ```json
  {
    "$schema": "https://openapi.vercel.sh/vercel.json",
    "functions": {
      "app/main.py": { "maxDuration": 30 }
    }
  }
  ```
...
> ⚠️ **Common failure:** an earlier version of this guide's `vercel.json`
> included `"runtime": "python3.12"`, which fails at build time with
> `Error: Function Runtimes must have a valid version, for example
> now-php@1.0.0`. Vercel's runtime strings need a `package@version` format
> it recognizes — but for FastAPI you don't need to set one at all, since
> zero-config detection handles it. Delete the `runtime` key (and the
> `rewrites` block, also unnecessary — FastAPI is served as a single
> Function that routes everything internally) if you hit this error.
...
|---|---|---|
| Build fails: `Error: Function Runtimes must have a valid version, for example now-php@1.0.0` | `vercel.json` has an invalid `runtime` field (e.g. `"python3.12"`) | Remove the `runtime` key entirely — FastAPI at `app/main.py` is zero-config auto-detected. Keep only `functions`/`maxDuration` if needed |
